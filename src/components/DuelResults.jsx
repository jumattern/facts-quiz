...
export default function DuelResults({ duel, lang, onBack, onHome }) {
  ...
  return (
    <div className="results animate-in">
      ...
      <div className="results-card">
        ...
        <h2 className="results-title">{t(lang, 'cityDuel')(duel.city)}</h2>

        {winner === 'tie' ? (
          <p className="duel-winner-text">{t(lang, 'itsTie')}</p>
        ) : (
          <p className="duel-winner-text">
            {t(lang, 'wins')(winner === 'challenger' ? c.name : o.name)}
          </p>
        )}
        ...
        <div className="duel-breakdown">
          <h3>{t(lang, 'questionByQuestion')}</h3>
          {c.answers.map((cAnswer, i) => {
            ...
            return (
              <div key={i} className="duel-q-row">
                ...
                <span className="duel-q-vs">{t(lang, 'vs').toLowerCase()}</span>
